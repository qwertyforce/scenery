    ...
    if (req.session?.user_id && !isNaN(booru_import_id) && allowed_boorus.includes(booru)) {
        ...
        if (user.isAdmin) {
            ...
            try {
                ...
                const img = await db_ops.image_ops.find_image_by_booru_id(booru, booru_import_id)
                if (img) {
                ...
                const phash = await image_ops.calculate_phash(image.data)
                const orientation = image_ops.get_orientation(booru_image_data.height, booru_image_data.width)
                ...
                await image_ops.calculate_image_features(new_image_id, image.data, phash)
